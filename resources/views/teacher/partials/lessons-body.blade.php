...
<div class="filterbar">
  <select data-refetch="#lessons-body" data-nav-param="class_id" data-nav-base="{{ $base }}">
    @foreach ($classList as $c)
      <option value="{{ $c->id }}" @selected($class && $c->id === $class->id)>{{ $c->name }}</option>
    @endforeach
  </select>
  <div class="weeknav" style="margin:0">
    <a class="btn ghost sm" href="{{ $base }}?class_id={{ $class?->id }}&week={{ $weekStart->copy()->subWeek()->toDateString() }}" data-refetch="#lessons-body">◀</a>
    <span class="wlabel" style="padding:0 6px">{{ $weekStart->format('d/m') }} – {{ $weekEnd->format('d/m') }}</span>
    <a class="btn ghost sm" href="{{ $base }}?class_id={{ $class?->id }}&week={{ $weekStart->copy()->addWeek()->toDateString() }}" data-refetch="#lessons-body">▶</a>
  </div>
</div>
...
@else
  <form method="POST" action="{{ route('teacher.lessons.save', [], false) }}" data-refetch="#lessons-body">
    @csrf
